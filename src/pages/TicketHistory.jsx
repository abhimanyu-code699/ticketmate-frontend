...
const TicketHistory = () => {
  ...
  const [selectedType, setSelectedType] = useState("uploaded");
  const [checkingTicket, setCheckingTicket] = useState({});
  ...
  // ⭐ Updated API Caller with ticket ID
  const checkTicketStatus = async (ticket) => {
    ...
    try {
      ...
      if (res.ok) {
        // ⭐ Store matched user info into ticket entry
        setHistory((prev) =>
          prev.map((t) =>
            t.id === ticket.id
              ? {
                  ...
                  matchedUserName: data.user?.name || null,
                  matchedUserPhone: data.user?.phone || null,
                }
              : t
          )
        );
        ...
        setStatusMessage((prev) => ({
          ...prev,
          [ticket.id]: data.message || "Ticket matched successfully",
        }));
      } else {
        setStatusMessage((prev) => ({
          ...prev,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredTickets.map((ticket) => (
              <div
                ...
                onClick={() => checkTicketStatus(ticket)}
              >
